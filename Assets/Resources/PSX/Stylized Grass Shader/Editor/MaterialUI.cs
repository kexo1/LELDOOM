...
namespace StylizedGrass
{
    ...
    public class MaterialUI : ShaderGUI
    {
        ...
        {
            EditorGUILayout.LabelField("Renderer integration:", EditorStyles.boldLabel);
            using (new EditorGUILayout.HorizontalScope(EditorStyles.helpBox))
            {
                EditorGUILayout.LabelField(ShaderConfigurator.CurrentConfig.ToString());
                if (GUILayout.Button("Change", GUILayout.MaxWidth(100f)))
                {
                    ...
                    {
                        menu.AddDisabledItem(new GUIContent("Vegetation Studio (Pro)"));
                    }
                    else
                    {
                        menu.AddItem(new GUIContent("Vegetation Studio (Pro)"), false, ShaderConfigurator.ConfigureForVegetationStudio);
                    }
                    if (ShaderConfigurator.CurrentConfig == ShaderConfigurator.Configuration.GPUInstancer)
                    {
                        menu.AddDisabledItem(new GUIContent("GPU Instancer"));
                    }
                    else
                    {
                        menu.AddItem(new GUIContent("GPU Instancer"), false, ShaderConfigurator.ConfigureForGPUInstancer);
                    }
                    if (ShaderConfigurator.CurrentConfig == ShaderConfigurator.Configuration.NatureRenderer)
                    {
                        menu.AddDisabledItem(new GUIContent("Nature Renderer"));
                    }
                    else
                    {
                        menu.AddItem(new GUIContent("Nature Renderer"), false, ShaderConfigurator.ConfigureForNatureRenderer);
                    }
